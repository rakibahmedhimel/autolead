...
def create_project(data: ProjectCreate, db: Session = Depends(get_db),
    ...
    project = Project(user_id=user.id, name=name, normalized_name=normalized, description=data.description)
    db.add(project)
    try:
        db.commit()
        db.refresh(project)
        ...
        existing = db.query(Project).filter(
            Project.user_id == user.id, Project.normalized_name == normalized
        ).first()
...
def get_project_companies(project_id: int, db: Session = Depends(get_db),
    ...
    query = db.query(Company).join(Job).filter(Job.project_id == project_id, Job.user_id == user.id)
    total = query.count()
