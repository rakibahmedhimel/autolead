...
class SpreadsheetJob(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "idempotency_key", name="uq_spreadsheet_user_idempotency"
        ),
    )
    ...
    sheets = relationship(
        "SpreadsheetSheet", cascade="all, delete-orphan", back_populates="job"
    )
...
class SpreadsheetSheet(Base):
    ...
    job_id: Mapped[int] = mapped_column(
        ForeignKey("spreadsheet_jobs.id", ondelete="CASCADE"), nullable=False
    )
    ...
    rows = relationship(
        "SpreadsheetRow", cascade="all, delete-orphan", back_populates="sheet"
    )
...
class SpreadsheetRow(Base):
    ...
    __table_args__ = (
        UniqueConstraint("sheet_id", "row_number", name="uq_spreadsheet_sheet_row"),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    sheet_id: Mapped[int] = mapped_column(
        ForeignKey("spreadsheet_sheets.id", ondelete="CASCADE"), nullable=False
    )
...
class CreditLedger(Base):
    ...
    __table_args__ = (
        UniqueConstraint("row_id", "field_name", name="uq_credit_row_field"),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_id: Mapped[int] = mapped_column(
        ForeignKey("users.id", ondelete="CASCADE"), nullable=False
    )
    spreadsheet_job_id: Mapped[int] = mapped_column(
        ForeignKey("spreadsheet_jobs.id", ondelete="CASCADE"), nullable=False
    )
    row_id: Mapped[int] = mapped_column(
        ForeignKey("spreadsheet_rows.id", ondelete="CASCADE"), nullable=False
    )
