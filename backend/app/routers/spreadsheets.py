...
from backend.app.models.spreadsheet import (
    CreditLedger,
    SpreadsheetJob,
    SpreadsheetRow,
    SpreadsheetSheet,
...
def owned_job(db, job_id, user):
    job = (
        db.query(SpreadsheetJob)
        .filter(SpreadsheetJob.id == job_id, SpreadsheetJob.user_id == user.id)
        .first()
    )
    if not job:
        raise HTTPException(status_code=404, detail="Spreadsheet job not found")
    return job
...
async def upload(
    ...
    existing = (
        db.query(SpreadsheetJob)
        .filter(
            SpreadsheetJob.user_id == user.id,
            SpreadsheetJob.idempotency_key == idempotency_key,
        )
        .first()
    )
    if existing:
    ...
    for position, (name, headers, rows) in enumerate(parsed):
        sheet = SpreadsheetSheet(
            job_id=job.id, name=name, position=position, headers=headers
        )
        db.add(sheet)
        db.flush()
...
def google_sheet(
    ...
    existing = (
        ...
        .filter(
            SpreadsheetJob.user_id == user.id,
            SpreadsheetJob.idempotency_key == data.idempotency_key,
        )
        .first()
    )
    if existing:
        return describe(existing)
    match = re.fullmatch(
        r"https://docs\.google\.com/spreadsheets/d/([A-Za-z0-9_-]+)(?:/.*)?", data.url
    )
    if not match:
        raise HTTPException(status_code=422, detail="Use a public Google Sheets link")
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="The Google Sheet could not be downloaded. Confirm it is public.",
        )
    job = SpreadsheetJob(
        user_id=user.id,
        ...
        "sheets": [
            {
                "name": s.name,
                "headers": s.headers,
                "preview": [r.values for r in s.rows[:5]],
            }
            for s in job.sheets
        ],
    }
...
def enrich(
    ...
    if not job.mapping:
        raise HTTPException(
            status_code=422, detail="Map the website and output columns first"
        )
    rows = (
        db.query(SpreadsheetRow)
        .join(SpreadsheetSheet)
        .filter(
            SpreadsheetSheet.job_id == job.id,
            SpreadsheetRow.status.in_(["pending", "failed"]),
        )
        .order_by(SpreadsheetRow.id)
        .limit(limit)
        .all()
    )
    credits, failed = 0, 0
    for row in rows:
        ...
        try:
            ...
            db.rollback()
            row = db.query(SpreadsheetRow).filter(SpreadsheetRow.id == row.id).one()
            row.status, row.error = "failed", str(error)[:500]
            failed += 1
            db.commit()
    job = owned_job(db, job_id, user)
    job.processed_rows = (
        db.query(SpreadsheetRow)
        .join(SpreadsheetSheet)
        .filter(SpreadsheetSheet.job_id == job.id, SpreadsheetRow.status == "completed")
        .count()
    )
    job.failed_rows = (
        db.query(SpreadsheetRow)
        .join(SpreadsheetSheet)
        .filter(SpreadsheetSheet.job_id == job.id, SpreadsheetRow.status == "failed")
        .count()
    )
    job.credits_used = (
        db.query(CreditLedger).filter(CreditLedger.spreadsheet_job_id == job.id).count()
    )
...
def download_xlsx(
    ...
    job = owned_job(db, job_id, user)
    sheets = [
        (s.name, s.headers, [r.values for r in s.rows])
        for s in sorted(job.sheets, key=lambda x: x.position)
    ]
    return StreamingResponse(
        io.BytesIO(workbook_bytes(sheets)),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": f'attachment; filename="spreadsheet-job-{job.id}.xlsx"'
        },
    )
...
def download_csv(
    ...
    def csv_text(sheet):
        ...
        for row in sheet.rows:
            writer.writerow(
                {key: safe_cell(row.values.get(key)) for key in sheet.headers}
            )
        return output.getvalue()

    if len(job.sheets) == 1:
        return StreamingResponse(
            iter([csv_text(job.sheets[0])]),
            media_type="text/csv",
            headers={
                "Content-Disposition": f'attachment; filename="spreadsheet-job-{job.id}.csv"'
            },
        )
    archive = io.BytesIO()
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="spreadsheet-job-{job.id}-csv.zip"'
        },
    )
...
def download_project(
    ...
    jobs = (
        ...
        .order_by(SpreadsheetJob.created_at)
        .all()
    )
    sheets = []
    for job in jobs:
        ...
        for source in sorted(job.sheets, key=lambda item: item.position):
            ...
            for row in source.rows:
                rows.append({"source_sheet": source.name, **row.values})
        if len(job.sheets) > 1:
            headers = ["source_sheet", *headers]
        sheets.append((f"Job {job.id}", headers, rows))
    return StreamingResponse(
        io.BytesIO(workbook_bytes(sheets)),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": f'attachment; filename="project-{project_id}.xlsx"'
        },
    )
